...
metadata <- broad_metadata %>%
  ...
  mutate(celltype_level3 = case_when(
    is.na(celltype_level3) ~ celltype_level1,
    TRUE ~ celltype_level3
  )) %>%
  mutate(isMultiplet = ifelse(celltype_level3 == "Doublets", 1, 0)) %>%
  mutate(celltype_level2 = case_when(
    celltype_level1 == "Epithelial cells" & celltype_level3 == "Epithelial cells" ~ "Pilosebaceous & Eccrine",
    celltype_level1 == "Epithelial cells" & celltype_level3 != "Epithelial cells" ~ "Keratinocyte",
